...
describe("getRarity", () => {
  describe("★3 虹", () => {
    it("watchersCount=22000 ちょうどで r3（単独閾値の境界値）", () => {
    ...
    it("watchersCount=10500 + satisfaction=83 で r3（ハイブリッド条件）", () => {
      expect(getRarity(10500, 83)).toBe("r3");
    });

    it("watchersCount=10500 + satisfaction=82.9 では r3 にならない", () => {
      expect(getRarity(10500, 82.9)).toBe("r2");
    });

    it("watchersCount=10499 + satisfaction=83 では r3 にならない", () => {
      expect(getRarity(10499, 83)).toBe("r2");
    });

    it("satisfactionRate が 0-1 スケールでも正規化される（0.83 → 83%扱い）", () => {
      expect(getRarity(10500, 0.83)).toBe("r3");
    });

    it("satisfactionRate=1.0（0-1スケールの100%）でも r3", () => {
      expect(getRarity(10500, 1.0)).toBe("r3");
    });
  });

  describe("★2 金", () => {
    it("watchersCount=5800 ちょうどで r2（境界値）", () => {
      expect(getRarity(5800, null)).toBe("r2");
    });

    it("watchersCount=10000 + satisfaction=100 でも r2（ハイブリッド閾値未満）", () => {
  ...
  describe("★1 青", () => {
    it("watchersCount=5799 で r1（境界値）", () => {
      expect(getRarity(5799, null)).toBe("r1");
    });
